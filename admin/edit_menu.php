<?php
    if($id !=""){
        $sel = $dbo->select("tbmenu where idmenu=$id");
        foreach($sel as $row){
    ...
    if(isset($_POST['simpan'])){
        ...
        if($foto == ""){
            $up = $dbo->update("tbmenu","nama_menu='$nama_menu',deskripsi='$deskripsi', harga='$harga', idkategori='$kategori'","idmenu=$idmenu");  
        }else{
            $nama_foto = date('YmdHis').$_FILES['foto']['name'];
            $nama_tmp = $_FILES['foto']['tmp_name'];
            $folder = '../img/';
            move_uploaded_file($nama_tmp,$folder.$nama_foto);
            $up = $dbo->update("tbmenu","nama_menu='$nama_menu',deskripsi='$deskripsi', harga='$harga', idkategori='$kategori',foto='$nama_foto'","idmenu=$idmenu");
        }
        if($up){
            ?>
            <script>
    ...
    <form action="?hal=edit_menu" method="post" enctype="multipart/form-data">
        <table>
            ...
            <tr>
                ...
                <td>:</td>
                <td>
                    <select name="kategori" id=""required><option value="">Pilih Kategori</option>
                    <?php
                    $kat = $dbo->select("tbkategori");
