<?php
    if($id !=""){
        $sel = $dbo->select("tbpetugas where idpetugas=$id");
        foreach($sel as $row){
    ...
    if(isset($_POST['simpan'])){
        ...
        if($password1 == ""){
            $up = $dbo->update("tbpetugas","nama_petugas='$nama_petugas',alamat='$alamat', no_hp='$no_hp', username='$username',role='$role'","idpetugas=$idpetugas");  
        }else{
            $pass = password_hash($password1, PASSWORD_DEFAULT);
            $up = $dbo->update("tbpetugas","nama_petugas='$nama_petugas',alamat='$alamat', no_hp='$no_hp', username='$username',role='$role' password='$pass'","idpetugas=$idpetugas");  
        }
        if($up){
            ?>
            <script>
