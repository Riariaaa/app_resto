<?php
    if($id != ""){
        $sel = $dbo->select('tbkategori where idkategori='.$id);
        foreach($sel as $data){
    ...
    if(isset($_POST['simpan'])){
        extract($_POST);
        $up = $dbo->update('tbkategori', "kategori='$kategori'", "idkategori = '$idkategori'");
        if($up){
            ?>
                    <script>
...
<div class="data-input">
    ...
    <tr>
        <td>
            <button class="btn-add" type="submit" value="simpan" name="simpan"><i class="fa fa-save"></i>Simpan</button>
        </td>
    </tr>
    </table>
    </form>
